# 📁 Esquemas-Asociativos-Gobernacion

> Proyecto del Centro de Valor Público – Universidad EAFIT

---

## 📝 Descripción

Apoyo a la Gobernación en el diseño y consolidación de esquemas asociativos territoriales (provincias, áreas metropolitanas, RAP).

---

## 🎯 Objetivos

- Caracterizar los esquemas asociativos territoriales existentes.
- Proponer modelos de gobernanza supramunicipal.
- Acompañar técnicamente a la Gobernación.

---

## 📊 Estado del proyecto

- **Estado actual:** 🟡 En curso
- **Fecha de inicio:** _por completar_
- **Fecha estimada de cierre:** _por completar_
- **Avance estimado:** _%_

> Actualiza este bloque cada vez que cambie el estado del proyecto.  
> Estados posibles: 🟢 Finalizado · 🟡 En curso · 🟠 En pausa · 🔴 Cancelado

---

## 👥 Responsable(s)

| Rol | Nombre | Correo |
|-----|--------|--------|
| Líder del proyecto | _por completar_ | _@eafit.edu.co_ |
| Coordinador(a) | _por completar_ | _@eafit.edu.co_ |
| Equipo de apoyo | _por completar_ | — |

---

## 🗂️ Estructura interna

| Carpeta | Qué va aquí |
|---------|-------------|
| `01_Documentos-Administrativos/` | Contratos, convenios, actas de inicio, pólizas, propuestas. |
| `02_Bases-de-Datos/` | Datos crudos, datos procesados, diccionarios de variables. |
| `03_Informes-y-Analisis/` | Informes parciales, análisis técnicos, productos intermedios. |
| `04_Entregables-Finales/` | Productos finales aprobados y entregados al cliente. |
| `05_Seguimiento-Proyecto/` | Cronogramas, actas de reunión, bitácoras, reportes de avance. |
| `06_Material-de-Apoyo/` | Presentaciones, infografías, fotos, videos, material de difusión. |

---

## 🔗 Enlaces útiles

- 📋 **Tablero Notion del proyecto:** _pegar enlace_
- 📅 **Calendario / cronograma:** _pegar enlace_
- 📂 **Carpeta original (Drive / OneDrive):** _pegar enlace si aplica_

---

## 📌 Notas

_Espacio para observaciones del equipo, decisiones importantes o pendientes destacados._
